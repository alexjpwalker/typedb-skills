...
- If something in the real world can exist independently (person, project, movie), make it an entity. If something only exists because of a connection between other things (product order, group membership, access grant), make it a relation. If something is just a value (size, date, name), make it an attribute.
- Use entities when a concept exists independently (and is not trivially representable by a scalar value). Entities represent the basic building blocks of any TypeDB schema.
...
## Naming of types
- TypeQL reads most elegantly when every type (entity, relation, role, and attribute) is named (aka: labeled) with a noun.
- All types: querying `match ... isa ...`, this reads as "search for: X is a Y" where Y is the type label.
...
- Use the `@abstract` annotation whenever it is applicable to define abstract supertype entities where only their subtypes are allowed to have instances.
...
- On many relations you will likely want to change the default `@card(0..1)` to `@card(1)` on every `relates` role type unless they are designed to be optional roles
- The only way to encode a 'required' connection is with `@card(1)` or higher counts eg. `@card(1..)`  (note: `@key` implies cardinality 1 as well). You will likely want to set this on many attribute `owns` declarations.

## Constraints
- It can be useful to define local `@values` or `@range` or other value restrictions at the `owns` level instead of the `attribute` definition level, particularly when an attribute has a fairly generic name and means different things in different contexts, such as `status`.
...
Judicious use of functions will help minimize code duplication and optimize for readability and maintainability, especially in larger schemas.
